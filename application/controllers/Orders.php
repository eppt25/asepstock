	...
	public function fetchOrdersData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if($value['paid_status'] == 1) {
				$paid_status = '<span class="label label-success">จ่ายแล้ว</span>';	
			}
			else {
				$paid_status = '<span class="label label-warning">ยังไม่จ่าย</span>';
			}
			...
			$result['data'][$key] = array(
				...
				$date_time,
				$count_total_item,
				$value['net_amount'] .' '. $company['currency'],
				$paid_status,
				$buttons
			);
		} // /foreach
		...
		if($id) {
			...
			<div class="wrapper">
			  <section class="invoice">
			    ...
			    <!-- /.row -->
				<button type="button" onclick="window.location=\''.base_url('orders').'\'" class="btn btn-default"><i class="fa fa-arrow-left"></i></button>
			  </section>
			  <!-- /.content -->
			</div>
		</body>
